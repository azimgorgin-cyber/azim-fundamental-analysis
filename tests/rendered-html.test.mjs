...
test("server-renders the Azim analysis dashboard", async () => {
  ...
  assert.match(html, /خلاصه عملکرد/);
  assert.match(html, /انتخاب دوره نمایش عملکرد/);
  assert.match(html, />فصلی</);
  assert.match(html, />سالانه</);
  assert.match(html, /سود خالص ۱۴۰۳/);
  ...
  assert.match(html, /تطبیق فروش ۱۴۰۴/);
